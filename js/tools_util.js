...
function initObdUI(){
  if(!$('obdGoBtn'))return;
  $('obdGoBtn').addEventListener('click',async()=>{
    const code=$('obdCode').value.trim();
    const vehicle=$('obdVehicle').value.trim();
    const status=$('obdStatus'),box=$('obdResult');
    if(!code){toast('اكتب كود الفحص أولًا (مثال: P0301)');return}
    box.style.display='none';box.className='result-box';
    status.style.color='var(--accent-amber)';status.textContent='⏳ جارٍ التشخيص…';
    try{
      const res=await obdDiagnoseRun(code,vehicle,s=>{status.textContent='⏳ '+s});
      status.textContent='';
      box.style.display='block';
      box.innerHTML=renderMarkdown(obdRenderReport(res));
      enhanceCodeBlocks(box);
    }catch(e){status.style.color='var(--accent-rose)';status.textContent='✗ '+friendlyError(e)}
  });
}
