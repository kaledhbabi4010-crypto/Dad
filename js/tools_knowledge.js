...
function initCryptoUI(){
  ...
  cryptoGo();
  /* يشتغل فقط طول ما تبويب العملات ظاهر فعليًا وواجهة الموقع مفتوحة —
     يوقف نفسه تلقائيًا لما تغادر الصفحة عشان ما يستهلك CoinGecko بلا داعٍ */
  if(cryptoInterval) clearInterval(cryptoInterval);
  cryptoInterval=setInterval(()=>{
    const view=document.getElementById('view-crypto');
    const visible=view && view.classList.contains('active') && document.visibilityState==='visible';
    if(visible) cryptoGo();
  },120000);
}
