  ...
  try{
    for(let round=1; round<=MAX_ROUNDS; round++){
      ...
      if(type==='html'){
        ...
        const status2=$('abStatus');
        status2.textContent='⏳ جولة '+round+' — جارٍ تشغيل الكود فعليًا (اختبار تنفيذ حقيقي، مو نحوي فقط)…';
        const ready = typeof ensurePyodide==='function' ? await ensurePyodide(status2) : false;
        if(!ready){ status2.textContent='⚠ تعذر تحميل بايثون للاختبار — الكود مُتاح لكن غير مُختبر تنفيذيًا'; break; }
        let runError=null;
        try{
          const timeout=new Promise((_,rej)=>setTimeout(()=>rej(new Error('انتهت المهلة (8 ثواني) — قد يحتوي الكود على حلقة لا نهائية')),8000));
          pyodide.globals.set('__ab_src__', code);
          await Promise.race([ pyodide.runPythonAsync('exec(__ab_src__)'), timeout ]);
        }catch(e){ runError = e && e.message ? e.message.split('\n').slice(-4).join(' ') : String(e); }
        if(!runError){ status2.textContent='✓ جولة '+round+' — الكود اجتاز اختبار تشغيل فعلي حقيقي بلا أخطاء'; break; }
        lastErrors=[runError];
        if(round===MAX_ROUNDS-1){ status2.textContent='⚠ لسا فيه خطأ تنفيذ بعد المحاولات — عرضته لك كأفضل نسخة متاحة، راجعه بنفسك: '+runError; break; }
        status2.textContent='⏳ جولة '+round+' — خطأ تنفيذ فعلي: '+runError+' — جارٍ التصحيح التلقائي…';
        prompt='هذا كود بايثون سبق وولّدته:\n```python\n'+code+'\n```\n\nشغّلته فعليًا ووصل هذا الخطأ الحقيقي وقت التنفيذ:\n'+runError+'\n\nأصلح السبب الجذري لهذا الخطأ مع الحفاظ على كل الوظيفة المطلوبة — أخرج الملف كاملًا مصححًا بنفس الشكل ```python، بدون أي شرح خارج الكتلة.';
      }
    }
    ...
    toast(lastErrors.length?'تم البناء مع تحذيرات — راجع الحالة أعلاه':(type==='python'?'تم البناء واختباره تنفيذيًا فعليًا ✓':'تم بناء المشروع ✓ وتحقق آليًا من سلامته'));
