...
async function fxSelectFiles(problem){
  ...
  const prompt='مستودع موقع ويب فيه هذي الملفات:\n'+list+'\n\nوصف المشكلة: "'+problem+'"\n\nأي الملفات يجب قراءتها لتشخيص وحل هذي المشكلة من جذورها؟ اكتب أسماء الملفات فقط (من القائمة أعلاه بالضبط)، كل اسم بسطر، بدون أي شرح. لا تختر أكثر من 3 إلا لو ضروري.';
...
async function fxTestConnection(){
  ...
  st.style.color='var(--accent-amber)';st.textContent='⏳ جارٍ التحقق من قدرة الوصول الفعلي لمستودع '+FX_REPO+'…';
  try{
    const r=await fetch('https://api.github.com/repos/'+FX_REPO,{headers:{Authorization:'Bearer '+token,'Accept':'application/vnd.github+json'}});
    ...
    if(r.status===404){st.style.color='var(--accent-rose)';st.textContent='✗ المستودع '+FX_REPO+' غير موجود أو المفتاح لا يملك صلاحية رؤيته';return}
    if(!r.ok){st.style.color='var(--accent-rose)';st.textContent='✗ رد غير متوقع من GitHub (HTTP '+r.status+')';return}
    const info=await r.json();
    const rr=await fetch('https://api.github.com/repos/'+FX_REPO+'/contents/index.html',{headers:{Authorization:'Bearer '+token,'Accept':'application/vnd.github+json'}});
