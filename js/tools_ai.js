...
function chatParsePythonRun(text){
  const t=text.trim();
  const hasCodeFence=/```/.test(t);
  const mentionsPython=/بايثون|python|py\b/i.test(t);
  const wantsRun=/^(شغل|شغّل|نفذ|نفّذ|run)(\s|$)/i.test(t);
  if(!wantsRun || !(hasCodeFence||mentionsPython)) return null; // لازم يذكر بايثون صراحة أو كود، عشان ما يتضارب مع أمر تبديل المحرك
  const m=t.match(/```(?:python|py)?\s*\n?([\s\S]*?)```/i);
  if(m) return m[1].trim();
  const rest=t.replace(/^(شغل|شغّل|نفذ|نفّذ|run)\s*(بايثون|python)?[:\s]*/i,'').trim();
  return rest || null;
}
async function chatTryRunPython(code){
  showView('python');
  if($('pyCode')) $('pyCode').value=code;
  const els=chatAppendMessage('assistant','🐍 نقلتك لأداة بايثون وشغّلت الكود مباشرة — شوف النتيجة بالأسفل هناك.');
  chatCurrent().messages.push({role:'assistant',content:'شغّلت الكود في أداة بايثون'});chatPersist();
  setTimeout(()=>{ if($('pyRunBtn')) $('pyRunBtn').click(); }, 200);
}
...
async function chatSend(text){
  ...
  const pyCode=(!priceIntent && !attach) ? chatParsePythonRun(prompt) : null;
  if(pyCode){
